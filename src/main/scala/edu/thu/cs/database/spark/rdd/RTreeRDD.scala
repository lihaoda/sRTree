...
private[spark] class RTreeRDD[T: ClassTag] (var prev: RDD[(RTree, Array[T])])
        ...
        aData.foreach( a => {
          ...
          bData.foreach( b => {
            apois.foreach(t => {
              ...
              val aindex = t._2
              rst += Tuple2((ap, adata(aindex)), func(partIds, (ap, adata(aindex)), b).toList)
            })
          })
        })
        rst
  ...
  def distJoin[W: ClassTag](rdd:RTreeRDD[W], dist:Double):RDD[((Point, T),(Point, W))] = {
    ...
    val rst = joinRDDWithPartition(rdd, joinParts, (parts:(Int, Int), a:(Point, T), b:(RTree, Array[W])) => {
      val point = a._1
      val adata = a._2
      val tree = b._1
      val bdatas = b._2
  ...
  def debugKnnJoin[W: ClassTag](rdd:RTreeRDD[W], k:Int) = {
    ...
    prev.zipPartitions(reparted)((aiter, biter) => {
      val arrayBuffer = new ArrayBuffer[((Point, T), Array[(Point, W)])]()
      aiter.foreach(t => {
        val aps = t._1.all
        val adatas = t._2
        aps.foreach(s => {
          biter.foreach(bt => {
            arrayBuffer += Tuple2((s._1.asInstanceOf[Point], adatas(s._2)), bt._1.kNN(s._1.asInstanceOf[Point], k, false).map(bs => {
